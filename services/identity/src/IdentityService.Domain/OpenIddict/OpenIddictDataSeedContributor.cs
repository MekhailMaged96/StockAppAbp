...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        var scopes = GetScopesFromConfig();
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var identitySwaggerRootUrl = configurationSection["IdentityService_Swagger:RootUrl"]?.TrimEnd('/');
        var consoleAndAngularClientRootUrl = configurationSection["IdentityService_App:RootUrl"]?.TrimEnd('/');
        var orderingServiceRootUrl = configurationSection["OrderingService:RootUrl"]?.TrimEnd('/');
        var inventoryServiceRootUrl = configurationSection["InventoryService:RootUrl"]?.TrimEnd('/');
        ...
        // Swagger Client
        var identitySwagerClient = configurationSection["IdentityService_Swagger:ClientId"];

        if (!identitySwagerClient.IsNullOrWhiteSpace())
        {

            await CreateApplicationAsync(
                applicationType: OpenIddictConstants.ApplicationTypes.Web,
                name: identitySwagerClient!,
                type: OpenIddictConstants.ClientTypes.Public,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                ...
                scopes: commonScopes,
                redirectUris: new List<string> { $"{identitySwaggerRootUrl}/swagger/oauth2-redirect.html" },
                clientUri: identitySwaggerRootUrl.EnsureEndsWith('/') + "swagger",
                logoUri: "/images/clients/swagger.svg"
            );
        }
        var orderingServiceClientId = configurationSection["OrderingService:ClientId"];
        if (!orderingServiceClientId.IsNullOrWhiteSpace())
        {
            ...
            OpenIddictConstants.GrantTypes.RefreshToken
                ...
                postLogoutRedirectUris: new List<string> { orderingServiceRootUrl },
                clientUri: orderingServiceRootUrl,
                logoUri: "/images/clients/service.svg"
            );
        }

        // Inventory Service
        var inventoryServiceClientId = configurationSection["InventoryService:ClientId"];
        if (!inventoryServiceClientId.IsNullOrWhiteSpace())
        {
            ...
            OpenIddictConstants.GrantTypes.RefreshToken
                ...
                postLogoutRedirectUris: new List<string> { inventoryServiceRootUrl },
                clientUri: inventoryServiceRootUrl,
                logoUri: "/images/clients/service.svg"
            );
        }
        ...
        string? logoUri = null)
    {
        ...
        else
        {
            ...
            client.ConsentType = application.ConsentType;
            client.ClientUri = application.ClientUri;
            client.LogoUri = application.LogoUri;
    ...
    private List<string> GetScopesFromConfig() => _configuration.GetSection("OpenIddict:Scopes").Get<List<string>>();
